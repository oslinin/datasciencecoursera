...
public class MaxFeeTxHandler {
    ...
    public Transaction[] handleTxs(Transaction[] possibleTxs) {
    	...
    	for (int k = 0 ; k<possibleTxs.length; k++) 
    		if (isValidTx(possibleTxs[k])) 
    			trs.add(possibleTxs[k]);
    	
    	double x = totalValue(trs);
    	int tot = (int)Math.ceil(x*1000);
    	...
    	for (j = 0; j< tot;  j++) {
    		if (j>=v) {
    			K[0][j]=v; 
    			T[0][j]=new Transaction2(trs.get(0), 0);
    		}
    	}
	...
	for (i=0; i<trs.size(); i++)
    	...
    	for(int i1 =0; i1< res.length; i1++) {
    		Transaction y = trs.get(i1);
    		res[i1] = y;
    		for (int j1 =0; j1<y.numInputs(); j1++){
        		Transaction.Input ti = y.getInput(j1);
    			UTXO txo = new UTXO(ti.prevTxHash, ti.outputIndex);
    ...
    public double totalValue(ArrayList<Transaction> trs){
    	...
    	Transaction[] x = new Transaction[trs.size()];
    	for (int i =0; i<x.length; i++) x[i]=trs.get(i);
    	return totalValue(x);
    }
