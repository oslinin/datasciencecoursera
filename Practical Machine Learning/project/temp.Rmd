...
The following is an application of machine learning to predict the validity of exercise performance based on sensor data.
...
Most data is sensor readings. The exception is the variable columns.
```{r , echo=T}
...
qplot(raw_timestamp_part_1, raw_timestamp_part_2, color=user_name, 
      data=traind[traind$user_name=="charles",], main = "Timestamp Columns: one user")
```

The field cvtd_timestamp seems to be a less granular version of raw_timestamp_part_1.
```{r , echo=T}
qplot(raw_timestamp_part_1, cvtd_timestamp, data=traind, color=user_name, main = "Timestamp Columns")
```
To see the result more clearly, we look at one user_name.
```{r , echo=T}
...
if (!exists("modFit.rpart")){
  ...
  print(modFit.rpart$finalModel) #variables handled appropriately
...
We will now fit a linear discriminant analysis model using the MASS package through the caret package.  
...
if (!exists("modFit.lda")){
  ...
  print(modFit.lda$finalModel) #variables handled appropriately
  print(confusionMatrix(trainp$classe, predict(modFit.lda,trainp))$overall['Accuracy'])
  save(modFit.lda, file="modFit.lda.RData")
} 
```

The accuracy is fairly high.  The model has also created indicated variables for the user_name, which is necessary for variable data, since the numerical value of the variable is meaningless.  We will use cross validation to attempt to improve out of sample performance, becuase the accuracy measure of the model above will be too optimistic for out of sample data.
